...
#method to load label_dist.txt, save it to working directory
def oversample():

    label_dist = genfromtxt('label_dist.txt', dtype=(int,int), delimiter='\t')
    label = []
    for i in range (label_dist.shape[0]):
        if (label_dist[i][1] < 15): #define the threshold to oversample instance
            label.append(label_dist[i][0])
    return label

#method to run augmentation on training data
def runAugment():
    from numpy import genfromtxt
    training_data_all = genfromtxt('train.csv', delimiter=',')

    training_inputs = (training_data_all[:, 9:])
    training_outputs = np.reshape(training_data_all[:, 1].astype(int),(50282,1))

        #import data to be augmented
    label = oversample()
    count = 0
    aug = []
    for i in range(training_data_all.shape[0]):
        # list has no contains syntax, only below
        if training_data_all[i][1] in label:
            aug.append(training_data_all[i])
            count += 1

    data_to_aug = np.reshape(aug, (count,438))
        # np.vstack([data_to_aug,training_data_all[i]])

    training_inAugment = training_inputs
    training_outAugment = training_outputs

    for j in range(data_to_aug.shape[0]):
        aug_input = data_to_aug[j,9:]
        aug_out = np.reshape(data_to_aug[j,1],(1,1))
        #reshape can change a list into array
        image = np.reshape(aug_input,(33,13))
        height = image.shape[0]
        width = image.shape[1]
        for i in range(20): # parameter to randomly generate new training data based on mirror, scaling, rotate
            augmenter = ImageAugmenter(width, height, hflip=True, vflip=True, scale_to_percent=1.3,
                                       scale_axis_equally=False, rotation_deg=25, shear_deg=10, translation_x_px=5,
                                       translation_y_px=5)  # random seed
            augmented_images = augmenter.augment_batch(np.array([image], dtype=np.uint8))
            img = np.reshape(augmented_images, (1,429))  # important, here is 33*13
            training_inAugment = np.vstack([training_inAugment,img])
            training_outAugment = np.vstack([training_outAugment,aug_out])

    return (training_inAugment, training_outAugment)
